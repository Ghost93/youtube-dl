...
class CBSNewsIE(CBSIE):
    ...
    def _real_extract(self, url):
        ...
        entries = []
        for embed_url in re.findall(r'<iframe[^>]+data-src="(https?://(?:www\.)?cbsnews\.com/embed/video/[^#]*#[^"]+)"',
                                    webpage):
        ...
        if entries:
            return self.playlist_result(
                ...
                playlist_description=self._html_search_meta(['og:description', 'twitter:description', 'description'],
                                                            webpage))
