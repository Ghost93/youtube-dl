...
class URPlayIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for quality_attr, quality, preference in (('', 'sd', 0), ('_hd', 'hd', 1)):
            file_http = urplayer_data.get('file_http' + quality_attr) or urplayer_data.get(
                'file_http_sub' + quality_attr)
            if file_http:
        ...
        return {
            ...
            'timestamp': unified_timestamp(
                self._html_search_meta(('uploadDate', 'schema:uploadDate'), webpage, 'timestamp')),
